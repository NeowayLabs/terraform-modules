...
resource "azurerm_network_security_rule" "rules" {
  ...
  source_port_ranges          = "${split(",", replace(  "${lookup(var.rules[count.index], "source_port_range", "*" )}") )}"
  destination_port_ranges     = "${split(",", replace(  "${lookup(var.rules[count.index], "destination_port_range", "*" )}") )}"
  ...
  description                 = "${lookup(var.rules[count.index], "description", "Security rule for ${lookup(var.rules[count.index], "name")}")}"
  resource_group_name         = "${azurerm_resource_group.nsg.name}"
